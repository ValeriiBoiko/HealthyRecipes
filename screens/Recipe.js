...
function Recipe({favorites, ...props}) {
  ...
  if (recipe.state === State.IN_PROGRESS || id !== params.recipeId) {
    ...
    <ScrollView
      ...
      <View>
        ...
        <Pressable style={styles.saveButton} onPress={onSave}>
          <Icon
            name={isSaved ? 'heart' : 'heart-empty'}
            color={colors.error}
            size={wp(24)}
          />
        ...
        <View style={[styles.card, {marginTop: wp(-40)}]}>
          ...
          <View style={styles.recipeInfo}>{infoItems}</View>
        </View>
      </View>
      ...
      <View style={[styles.sectionCard, styles.card]}>
        ...
        <Text style={styles.sectionSubTitle}>
          Total ingredient count : {ingredients.length}
        </Text>
      ...
      <View style={[styles.sectionCard, styles.card]}>
        ...
        <Text style={styles.sectionSubTitle}>
          No of steps is : {instructions.length}
        </Text>
...
const getStyles = (colors, insets) =>
  StyleSheet.create({
    ...
    saveButton: {
      ...
      right: wp(20),
      backgroundColor: colors.cardWithOpacity(0.8),
    },
    saveLabelWrapper: {
      marginLeft: 8,
      alignItems: 'center',
